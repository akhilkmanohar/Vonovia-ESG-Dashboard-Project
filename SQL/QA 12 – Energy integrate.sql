/* =========================================================================================
   QA 12 – Energy integrate (single sheet)
   Sections:
     - proc_exists     : confirm energy proc is present
     - counts          : row counts for core.energy_yearly
     - totals_by_year  : mart totals (top 20) for a quick smoke test
   ========================================================================================= */
SET NOCOUNT ON;

WITH proc_exists AS (
    SELECT 'proc_exists' AS section,
           'core.sp_refresh_energy_yearly' AS k1,
           CASE WHEN OBJECT_ID('core.sp_refresh_energy_yearly','P') IS NOT NULL THEN 'yes' ELSE 'no' END AS k2,
           CAST(NULL AS nvarchar(4000)) AS k3,
           CAST(NULL AS nvarchar(4000)) AS k4
),
counts AS (
    SELECT 'counts' AS section,
           'core.energy_yearly' AS k1,
