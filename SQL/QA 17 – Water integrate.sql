/* =========================================================================================
   QA 17 – Water integrate (single sheet)
   Sections:
     - proc_exists     : confirm water proc is present
     - counts          : row counts for core.water_yearly (min/max year)
     - totals_by_year  : mart totals top 20 (m³)
   ========================================================================================= */
...
WITH proc_exists AS (
    SELECT 'proc_exists' AS section,
           'core.sp_refresh_water_yearly' AS k1,
           CASE WHEN OBJECT_ID('core.sp_refresh_water_yearly','P') IS NOT NULL THEN 'yes' ELSE 'no' END AS k2,
