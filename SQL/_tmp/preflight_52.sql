...
-- Cap MAXDOP to 1 for safer compile/run
BEGIN TRY
    ALTER DATABASE SCOPED CONFIGURATION SET MAXDOP = 1;
    PRINT ''[Preflight] Database-scoped MAXDOP set to 1.'';
END TRY BEGIN CATCH
...
-- Sweep obvious sleeping blockers (no open tran) that are blocking others
...
BEGIN
    ...
    BEGIN TRY
        DECLARE @cmd nvarchar(50) = N''KILL '' + CAST(@spid AS nvarchar(10));
        EXEC(@cmd);
